...
class PaymentIntentResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    protected static $openAPITypes = [
        ...
        'status' => '\PAYJPV2\Model\PaymentIntentStatus',
        'next_action' => 'object',
        'return_url' => 'string',
    ...
    public const OBJECT_PAYMENT_INTENT = 'payment_intent';
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('object', $data ?? [], 'payment_intent');
    ...
    */
    ...
    public function setAmountCapturable($amount_capturable)
    {
        if (is_null($amount_capturable)) {
            array_push($this->openAPINullablesSetToNull, 'amount_capturable');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('amount_capturable', $nullablesSetToNull);
            if ($index !== FALSE) {
    ...
    public function setAmountReceived($amount_received)
    {
        if (is_null($amount_received)) {
            array_push($this->openAPINullablesSetToNull, 'amount_received');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('amount_received', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
     ...
     * Sets client_secret
     *
     * @param string $client_secret このPaymentIntentのクライアントシークレットです。フロントエンドで公開APIキーと合わせて使用しPaymentIntentの情報を取得や支払い処理を行います。**この値はこのPaymentIntentの支払いを行う顧客以外へ公開しないでください。**また保存やログへの記録なども行わないでください。
     *
     * @return self
     */
     ...
     *
     * @param string[] $payment_method_types このPaymentIntentで使用できる支払い方法の種類（カードなど）のリストです。 指定しない場合は、PAY.JPは支払い方法の設定から利用可能な支払い方法を動的に表示します。
     *
     * @return self
     */
     ...
     * Gets status
     *
     * @return \PAYJPV2\Model\PaymentIntentStatus
     */
     ...
     * Sets capture_method
     *
     * @param \PAYJPV2\Model\CaptureMethod $capture_method 支払いの確定方法を指定します。  | 指定できる値 | |:---| | **automatic**: 顧客が支払いを承認すると、自動的に確定させます。 | | **automatic_async**: (デフォルト) 顧客が支払いを承認すると、非同期的に確定させます。レイテンシが改善されるため、capture_method=automaticよりも推奨されます。詳細については、統合ガイドをお読みください。 | | **manual**: 顧客が支払いを承認すると一旦確定を保留し、後で Capture API を使用して確定します。（すべての支払い方法がこれをサポートしているわけではありません）。 |
     *
     * @return self
     */
