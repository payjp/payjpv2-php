...
class PaymentFlowCreateRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     *
     * @param \PAYJPV2\Model\PaymentMethodOptionsRequest|null $payment_method_options このPaymentFlowに固有の支払い方法の設定
     *
     * @return self
     */
     ...
     *
     * @param \PAYJPV2\Model\PaymentMethodTypes[]|null $payment_method_types このPaymentFlowで使用できる支払い方法の種類（カードなど）のリストです。 指定しない場合は、PAY.JPは支払い方法の設定から利用可能な支払い方法を動的に表示します。
     *
     * @return self
     */
     ...
     * Sets capture_method
     *
     * @param \PAYJPV2\Model\CaptureMethod|null $capture_method 支払いの確定方法を指定します。  | 指定できる値 | |:---| | **automatic**: (デフォルト) 顧客が支払いを承認すると、自動的に確定させます。 | | **manual**: 顧客が支払いを承認すると一旦確定を保留し、後で Capture API を使用して確定します。（すべての支払い方法がこれをサポートしているわけではありません）。 |
     *
     * @return self
     */
     ...
     *
     * @param \PAYJPV2\Model\Usage|null $setup_future_usage このPaymentFlowの支払い方法で将来の支払いを行う意図があることを示します。<br><br>PaymentFlow に Customer を指定した場合、このパラメータを使って PaymentFlow を確定できます。その後、顧客が必要な操作を完了すると、支払い方法を Customer に紐付けることが可能です。また、Customer を指定しない場合でも、取引が完了した後に支払い方法を Customer に紐付けることはできます。
     *
     * @return self
     */
